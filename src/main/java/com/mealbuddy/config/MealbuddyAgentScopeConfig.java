 ...
 */
@Configuration
public class MealbuddyAgentScopeConfig {
    ...
    @Value("${mealbuddy.llm.main-model:qwen-max}")
    private String mainModelName;

    /** 轻量模型用于意图识别和澄清追问，默认使用 qwen-turbo。 */
    @Value("${mealbuddy.llm.light-model:qwen-turbo}")
     ...
     */
    @Bean("MealbuddyMainChatModel")
    public Model MealbuddyMainChatModel() {
     ...
     */
    @Bean("MealbuddyLightChatModel")
    public Model MealbuddyLightChatModel() {
        return DashScopeChatModel.builder()
                .apiKey(apiKey)
