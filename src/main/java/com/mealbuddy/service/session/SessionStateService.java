...
public class SessionStateService {
    ...
    public void save(SessionState state) {
        ...
        if (updated == 0) {
            throw new MealbuddyException("会话状态保存失败"); // 影响行数 0 说明 sessionId 不存在或并发冲突
        }
    }
    ...
    private SessionState fromRow(SessionRow row, SourceMode requestSourceMode) {
        try {
            ...
            throw new MealbuddyException("会话状态解析失败", e);
        }
    }
    ...
    private String toJson(Object value) {
        try {
            ...
            throw new MealbuddyException("会话状态 JSON 序列化失败", e);
        }
    }
