...
import com.mealbuddy.mapper.MealMapper;
import com.mealbuddy.model.MealItem;
import com.mealbuddy.model.MealItemRow;
...
public class MealService {
    ...
    public MealItem updatePersonalMeal(Long userId, Long mealId, MealRequest request) {
        ...
        if (updated == 0) {
            throw new MealbuddyException("个人餐食不存在或无权限修改");
        }
    ...
    public void deletePersonalMeal(Long userId, Long mealId) {
        ...
        if (deleted == 0) {
            throw new MealbuddyException("个人餐食不存在或无权限删除");
        }
    }
    ...
    private void validateMealRequest(MealRequest request) {
        if (request == null || request.name() == null || request.name().isBlank()) {
            throw new MealbuddyException("餐食名称不能为空");
        }
        SlotBundle slots = request.toSlots();
        if (slots.mealTime().isEmpty()) {
            throw new MealbuddyException("餐次至少选择一个标签");
        }
