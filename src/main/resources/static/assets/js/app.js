(function () {
    ...
    function currentRoute() {
        return (location.hash || "#/mealbuddy").slice(1).split("?")[0] || "/mealbuddy";
    }
    ...
    function render() {
        ...
        if (route === "/mealbuddy") {
            renderHome();
        } else if (route === "/mealbuddy/chat") {
            renderChat();
        } else if (route === "/mealbuddy/meals/personal") {
            renderPersonalMeals();
        } else if (route === "/mealbuddy/meals/public") {
            renderPublicMeals();
            ...
            <section class="hero">
                <div class="hero-panel">
                    ...
                    <div class="hero-actions">
                        <a class="btn primary" href="#/mealbuddy/chat">开始聊天推荐</a>
                        <a class="btn soft" href="#/mealbuddy/meals/personal">管理个人餐食</a>
                ...
                <aside class="grid stats">
                    ...
                    ${statCard("当前用户", MealbuddyApi.getUserId(), "所有请求会带上 X-User-Id")}
                </aside>
            </section>
            <section class="grid three" style="margin-top: 18px;">
                ${featureCard("聊天推荐", "按自然语言表达需求，页面会展示澄清问题、推荐卡片和反馈入口。", "#/mealbuddy/chat")}
                ${featureCard("餐食维护", "用标签多选维护自己的常吃餐食，后续推荐会优先从个人库检索。", "#/mealbuddy/meals/personal")}
    ...
    async function loadHomeStats() {
        ...
        try {
            const [personal, publicMeals] = await Promise.all([
                MealbuddyApi.listPersonalMeals(),
                MealbuddyApi.listPublicMeals()
            ]);
            state.home = {
                loaded: true,
                ...
                <aside class="grid">
                    ...
                    <div class="card">
                        ...
                        <div class="button-row">
                            <a class="btn soft" href="#/mealbuddy/meals/personal">维护餐食</a>
                            <a class="btn ghost" href="#/mealbuddy/meals/public">看公共库</a>
                        </div>
                    </div>
                </aside>
            </section>
    ...
    async function submitChat(form) {
        ...
        try {
            if (!state.chat.sessionId) {
                const session = await MealbuddyApi.createSession();
                state.chat.sessionId = session.sessionId;
            }
            const response = await MealbuddyApi.chat({
    ...
    async function renderPersonalMeals() {
        if (!state.slotOptions) {
            ...
            if (currentRoute() !== "/mealbuddy/meals/personal") {
                return;
            }
        }
    ...
    async function ensurePersonalMeals(force) {
        ...
        try {
            state.personalMeals = await MealbuddyApi.listPersonalMeals();
    ...
    async function ensureSlotOptions() {
        ...
        try {
            state.slotOptions = await MealbuddyApi.slotOptions();
        } catch (error) {
    ...
    async function saveMeal(form) {
        ...
        try {
            await guard(async () => {
                if (id) {
                    return MealbuddyApi.updatePersonalMeal(id, payload);
                }
                return MealbuddyApi.createPersonalMeal(payload);
            }, id ? "餐食已更新" : "餐食已创建");
    ...
    async function deleteMeal(id) {
        ...
        await guard(async () => {
            await MealbuddyApi.deletePersonalMeal(id);
            await ensurePersonalMeals(true);
            renderPersonalMeals();
        }, "餐食已删除");
    }
        ...
        app.innerHTML = `
            <section class="section">
                <div class="card-title">
                    ...
                    <a class="btn primary" href="#/mealbuddy/chat">去聊天推荐</a>
                </div>
    ...
    async function ensurePublicMeals(force) {
        ...
        try {
            state.publicMeals = await MealbuddyApi.listPublicMeals();
            state.home.loaded = false;
            if (currentRoute() === "/mealbuddy/meals/public") {
    ...
    async function searchTraces(form) {
        ...
        try {
            if (state.traces.filters.sessionId) {
                state.traces.rows = await MealbuddyApi.listSessionTraces(state.traces.filters.sessionId, state.traces.filters.limit);
            } else {
                state.traces.rows = await MealbuddyApi.listTraces({
    ...
    async function selectTrace(traceId) {
        await guard(async () => {
            state.traces.selected = await MealbuddyApi.getTrace(traceId);
            renderTraces();
        });
    }
    async function saveTraceLabel(form) {
        ...
        await guard(async () => {
            await MealbuddyApi.labelTrace(traceId, payload);
            state.traces.selected = await MealbuddyApi.getTrace(traceId);
    ...
    async function runEvaluation(form) {
        ...
        renderEvaluations();
        try {
            state.evaluation.report = await MealbuddyApi.evaluate(state.evaluation.form);
        } catch (error) {
    ...
    async function saveFeedback(button) {
        await guard(async () => {
            await MealbuddyApi.saveFeedback({
    ...
    function initUserField() {
        userIdInput.value = MealbuddyApi.setUserId(MealbuddyApi.getUserId());
        userIdInput.addEventListener("change", () => {
            MealbuddyApi.setUserId(userIdInput.value);
            state.home.loaded = false;
            state.personalMeals = [];
            state.publicMeals = [];
            state.traces.rows = [];
